...
def train_model(model, Xtrain, Ytrain, Xval, Yval, id_to_char, max_epoch):
    ...
    for e in range(max_epoch):
        ...
        for n in range(num_batches):
            ...
            optim.step()
            # Accuracy
            char_count = torch.count_nonzero(Ytrain[s:e].flatten())
            total_chars += char_count.detach().cpu().numpy()
            pred = torch.argmax(out, dim =-1)
            total_train_accuracy +=  torch.sum(pred == Ytrain[s:e]).item()
    
        avg_train_accuracy = total_train_accuracy / total_chars
...
def gen_string(model, id_to_char, max_len=20, sample=True):
    ...
    for i in range(max_len):
        ...
        exp_preds=output_dist.cpu().detach().numpy()
        preds = exp_preds / np.sum(exp_preds,axis=0)
        
        # Random sampling from multinomial distribution
        if sample==True:
            preds=torch.tensor(preds,dtype=torch.float)
            topi = int(torch.multinomial(preds, 1)[0])   
        # Argmax sampling
        else:
            topi=int(np.argmax(preds))
        x=torch.tensor([[topi]])
        letter = id_to_char[topi]
        if i == max_len - 1 or letter=='.':
            break
        else:
            out_str += letter
